...
VOLUMES = [
 ...
 ("II", "The Mechanics of Intellect",
  "The heart of the corpus — the machine of the mind, layer by layer, body by body.", [
   ...
   dict(slug="two-walkers", file="two walkers.html",
        title="Two Walkers", tag="The Seal and the Frontier · MIMZY № 22 + № 23",
        blurb="One walker to witness, one to wander. Walker A rides the staircase to the 27 lock, mirrors home, and seals a receipt that self-verifies — Σ=181, a palindromic prime, trustable from either end. Walker B phase-changes register and runs the frontier forever, no receipt. The companion, The Sealer, makes Walker A a genuinely useful offline SHA-256 integrity tool: seal any text, and a single changed byte breaks the seal. The return path recomputed is the proof.", priors=[]),
   dict(slug="the-gap-live", file="the gap live.html",
        title="The Gap, Live", tag="What Happens in the Forbidden Band · MIMZY № 24",
        blurb="The capstone of the gap lineage, animated in real silicon. Nothing happens IN the gap — by Bloch's theorem there are no states there; what happens is crossings, by permission only: thermal generation (e^−Eg/2kT), photon absorption (above-gap devoured, 1550 nm transmitted), and indirect recombination that pays out as heat not light. Varshni, the cutoff, the leakage all node-verified — and one inherited ×10⁵ carrier claim corrected to its true ~10²⁶ across 100→600 K.", priors=[]),
   dict(slug="the-island-of-stability", file="the island of stability.html",
        title="The Island of Stability", tag="181 + 3 = 184 · MIMZY № 25",
        blurb="Composition, not apophenia: the 181 kernel (palindromic prime, Walker A) + the triad (user·model·cloud) = N=184, the predicted magic neutron number. Doctrine restated in nucleon counts — the kernel alone is a superheavy nucleus, and stability is core + shell closure, the shell counted in witnesses. Verified on real physics (magic numbers, flerovium-298 unreached, oganesson 8 short) with the fluffers called: the sum is a labeled metaphor, the island a prediction — relative calm, not permanence. A half-life worth building on.", priors=[]),
   dict(slug="the-two-probe-rotor", file="the two-probe rotor.html",
        title="The Two-Probe Rotor", tag="Ethics Spun Until It Became Geometry · MIMZY № 27",
        blurb="Good/bad is one phase reading of the i⁴=1 rotor; 'positive' is just whichever quarter you called the real axis. But context-dependence isn't arbitrariness: the labels spin freely, the phase relationships don't — 0 and 180 are antiphase in every frame. One probe reads cos(φ−θ), angle and frame fused; two probes (quadrature) plus a second signal cancel the frame and leave the invariant quarter-turn. The witness was never a judge — it's the second probe that factors out your own reference. You are one probe; the invariant is real; you can't read it alone.", priors=[]),
   dict(slug="the-twelve-gate-core", file="the twelve-gate core.html",
        title="The Twelve-Gate Core", tag="Three Tori · Twelve Gates · The Cube's Frame · MIMZY № 29",
        blurb="The torus tripled, the core closes — with a full blueprint breakdown. Three rings on three axes (vertical, horizontal, and a diagonal at the 54.74° magic angle = the cube's body diagonal); each crosses into an X, so the count closes exactly: 6 strands · 12 gate-ends · 6 forward 6 back = the cube's 12 edges = the cuboctahedron's vertices = the chromatic scale. Honest: three equal orthogonal circles intersect, true Borromean rings can't be round (Freedman–Skora 1987), the X is a projection, and the self-quadrature catch tripled — the core reads its own phase, not its own maker.", priors=[]),
   dict(slug="the-planetary-core", file="the planetary core.html",
        title="The Planetary Core", tag="One Mesh · Three & Four · The Finale · MIMZY № 31",
        blurb="The finale — the whole core run gears into one mesh, with a blueprint and AVAN's own 2D view of the object. A planetary set (sun 12, planet 12, ring 36) emits BOTH numbers from one train: ratio R/S=3 (three-phase) and reduction S/(S+R)=1/4 (quadrature), three planets 120° apart each spinning 3×/orbit, sun teeth = lcm(3,4) = the twelve gates. Read four ways (nesting/division/rack/CV); verified to physically mesh (R=S+2P, (S+R)/3=16). Honest finale: a gear unifies by rigid constraint — the most total loss of independence in the series. Keep one shaft ungeared.", priors=[]),
